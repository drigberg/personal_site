(function () {
  ...
  const FONT_COLOR = "#01295F";
  const PATH_COLOR = "white";
  const HIGHLIGHTED_COLOR = "#F3C178";
  ...
  function fetchTripData() {
    return fetch("/public/assets/data/trips.json").then((res) => res.json());
  }

  function fetchCitiesData() {
    return fetch("/public/assets/data/cities.json").then((res) => res.json());
  }

  Promise.all([fetchWorldData(), fetchTripData(), fetchCitiesData()]).then(
    ([world, tripData, citiesData]) => {
      function render() {
        ...
        function animateRouteForwards(r) {
          ...
          r.attr("stroke-dasharray", totalLength + " " + totalLength)
        ...
        function handleDragCircle(d) {
          return (
            "translate(" +
            projection([d.coordinates[0], d.coordinates[1]]) +
            ")"
          );
        }
        ...
        function pointIsOnOtherSideOfGlobe(coords) {
          var geoangle = d3.geo.distance(coords, projection.invert(center));
        ...
        function handleShowLabel(d) {
          if (
            d.tripId === highlightedTripId &&
            !pointIsOnOtherSideOfGlobe(d.coordinates)
          ) {
            return "1.0";
          } else {
            return "0";
        ...
        function highlightTrip(tripId) {
          ...
          pathG
            ...
            .moveToFront();
          pathG.selectAll(`.text-${tripId}`).moveToFront();
          pathG.selectAll("text").attr("opacity", handleShowLabel);
        }
        ...
        function hideAllTrips() {
          ...
          pathG.selectAll("circle").style({ fill: "white" });
          pathG.selectAll("text").attr("opacity", handleShowLabel);
        }
        ...
        function getCityCoords(citiesMapping, cityId) {
          if (Object.keys(citiesMapping).includes(cityId)) {
            ...
            console.error(
              `Cannot find ${cityId} in cities -- returning origin instead of failing`
            );
            return [0, 0];
          }
        }
        ...
        function getCityName(citiesMapping, cityId) {
          if (Object.keys(citiesMapping).includes(cityId)) {
            ...
            console.error(
              `Cannot find ${cityId} in cities -- returning id instead of failing`
            );
            return cityId;
          }
        }
        ...
        function drawTrips(trips, citiesMapping) {
          const isAlanMode = getQueryVariable("alan") === "1";
          console.log("ALAN:", isAlanMode);

          for (const trip of trips) {
            if (isAlanMode && !trip.alan) {
              continue;
            }
            ...
            const route = pathG
              ...
              .data([
                {
                  type: "LineString",
                  coordinates: trip.locations.map((location) =>
                    getCityCoords(citiesMapping, location.city)
                  ),
                },
              ])

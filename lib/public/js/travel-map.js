      ...
      pathG.append("path")
        .data([{
          ...
          coordinates: trip.locations.map(
            location => {
              if (Object.keys(citiesMapping).includes(location.city)) {
                return citiesMapping[location.city];
              } else {
                console.error(`Cannot find ${location.city} in cities -- returning origin instead of failing`)
                return [0, 0]
              }
            })
        }])
